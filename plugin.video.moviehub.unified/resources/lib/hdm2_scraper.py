...
Fixed parsing patterns based on actual site structure
"""
...
def _extract_listing(html):
    """Extract movies from listing page using correct patterns"""
    items = []
    seen = set()

    # Pattern 1: article.card structure
    for article in re.findall(r'<article[^>]*class="[^"]*card[^"]*"[^>]*>.*?</article>', html, re.DOTALL):
        # Extract title and URL from h3.card-title > a
        a = re.search(r'<h3[^>]*class="[^"]*card-title[^"]*"[^>]*>\s*<a[^>]+href="([^"]+)"[^>]*>([^<]+)</a>', article, re.I)
        if not a:
            # Fallback: any h3 > a
        ...
        # Extract thumbnail
        thumb = ""
        img_m = re.search(r'<img[^>]+src="(https://image\.tmdb\.org/t/p/w342/[^"]+)"', article, re.I)
        if not img_m:
            img_m = re.search(r'<img[^>]+src="([^"]+)"[^>]*alt="[^"]*"', article, re.I)
        if img_m:
            thumb = img_m.group(1)

        # Extract year
        year = ""
        ym = re.search(r'<div[^>]*class="[^"]*meta[^"]*"[^>]*>\s*<span>(\d{4})</span>', article, re.I)
        if not ym:
    ...
    # Pattern 2: If no items found, try alternative patterns
    if not items:
        for a in re.finditer(r'<a[^>]+href="([^"]*(?:movies|tv-shows)/[^"]+)"[^>]*title="([^"]+)"', html):
            url = _abs(a.group(1))
            title = _clean_text(a.group(2))
            if url in seen or not title or len(title) < 3:
                continue
            seen.add(url)
            items.append({"title": title, "url": url, "thumb": "", "year": ""})

    # Pattern 3: Look for poster-link with aria-label
    if not items:
...
def search(query, page=1):
    ...
    for u in urls:
        if q in u.lower():
            ...
            results.append({"title": title, "url": u, "thumb": "", "year": ""})
    return results
...
def get_detail(url):
    ...
    # Extract sources - use correct patterns
    sources = []
    seen = set()

    # Pattern 1: data-source attribute on player options
    for emb in re.findall(r'<a[^>]+href="[^"]*#single"[^>]+data-source="([^"]+)"', html):
        emb = _abs(emb, url)
    ...
    # Pattern 2: hdm2.ink sources
    for emb in re.findall(r'(https://hdm2\.ink/play\?v=[^"\'\s<>\[\]]+)', html):
        if emb not in seen:
            seen.add(emb)
            sources.append({"label": "HD Source (hdm2.ink)", "url": emb, "host": "hdm2.ink"})

    # Pattern 3: prvs.top sources
    for emb in re.findall(r'(https://prvs\.top/[^"\'\s<>\[\]]+)', html):
        if emb not in seen:
            seen.add(emb)
            sources.append({"label": "Source (prvs.top)", "url": emb, "host": "prvs.top"})

    # Pattern 4: YouTube embeds (handle bracketed IDs)
    for emb in re.findall(r'(https://www\.youtube\.com/embed/[^"\'\s<>\[\]]+)', html):
        # Remove brackets from YouTube IDs
        emb = emb.replace("[", "").replace("]", "")
        if emb not in seen:
            seen.add(emb)
            sources.append({"label": "YouTube", "url": emb, "host": "youtube"})

    # Pattern 5: data-source / data-embed / data-link
